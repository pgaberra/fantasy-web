...
[2026-09-11] fantasy-web: robots.txt and sitemap.xml are checked against the router by a spec, not kept in step by hand — five routes added after #255 landed in neither file; a default-deny robots.txt (`Disallow: /` plus an allow-list) was rejected because it also blocks the bundle Googlebot needs to render the SPA and the `/s/` share links X's preview bot must reach, which is also why `/s/` stays crawlable.

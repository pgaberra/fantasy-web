...
[2026-09-11] fantasy-web: a saved row whose skater/goalie type disagrees with the pool is restarted from the player's own pool line where the table loads it, not guarded with `?? 0` in the cell or squared in the BFF reconciler — a zero-filled cell would draw a goalie's line under skater columns and rank it as a goalie, and the reconciler only sees rows it seeded, while model seeds, copies and imports reach the table without passing it.

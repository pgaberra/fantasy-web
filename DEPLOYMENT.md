...
| `MANUAL_RANKING_ENABLED` | Offers **Ranking** in the projection editor's toolbar, where a player type is switched from its projected stats to the order the owner types in the # column. `true` shows it; empty/anything-else keeps it hidden (default). A projection saved while it was on keeps its order: turning the flag back off hides the control and ranks the board by its projections again, it does not delete anything | `true` |
| `TIERS_ENABLED` | Shows **positional tiers**: the tier column and tier breaks in the projection editor, and the tier badges and tier strip in draft mode. Tiers are computed from the projection itself, so this is purely a display switch — nothing is stored and nothing is fetched. `true` shows them; empty/anything-else keeps them hidden (default) | `true` |
...
| `SHARED_NOTICE_ENABLED` | Draws the adopting pages' notices through the shared `<app-notice>` component instead of each page's own markup: the off-season notice, the "this projection changed while you were viewing it" line, and Team Power Rankings' two error lines. `true` uses the shared component; empty/anything-else leaves every one of them exactly as it shipped (default). Purely a display switch — nothing is stored or fetched, and only the off-season notice looks different with it on | `true` |

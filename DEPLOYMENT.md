...
| `PREMIUM_PRICE_REQUIRED` | Whether a build that sells Premium (`PAYMENTS_ENABLED=true` plus a Paddle token and price id) **fails** when the prerendered `/premium` has no price, because Paddle's pricing preview failed at build time. `true` fails the build; empty/anything-else only prints a warning in the build log and builds anyway (default). Left unset while Paddle has not approved the domain, since the live pricing preview gave the prod build no quote before that (v0.205.0). Set `true` on both apps once it has | `true` |
